...
) const
{
    ...
    volScalarField Re(max(Ur*phase1_.d()/phase2_.nu(), scalar(1.0e-6)));
    volScalarField Cds
    (
	...
	//Info << "Particle Re, max" << max(Re).value() 
	//		<< "min" << min(Re).value() << endl;

    return 0.75*Cds*phase2_.rho()*Ur*pow(alpha2, -2.65)/phase1_.d();
}


Foam::tmp<Foam::volScalarField> Foam::WenYu::Cd
(
    const volScalarField& Ur
) const
{
    ...
    return 0.75*Cd*phase1_.d()*(phase1_.rho()/phase2_.rho())/Ur;
}
